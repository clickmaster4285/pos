  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-linear-to-br from-slate-50 via-blue-50/30 to-purple-50/20 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden">
        <motion.div animate={{ x: [0, 100, 0], y: [0, -50, 0] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="absolute top-20 left-10 w-72 h-72 bg-linear-to-r from-blue-200/40 to-cyan-200/40 rounded-full blur-3xl" />
        <motion.div animate={{ x: [0, -80, 0], y: [0, 60, 0] }} transition={{ duration: 25, repeat: Infinity, ease: "linear", delay: 2 }} className="absolute bottom-20 right-10 w-96 h-96 bg-linear-to-r from-purple-200/30 to-pink-200/30 rounded-full blur-3xl" />
      </div>

      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md relative z-10">
        <motion.div className="text-center mb-8">
          <motion.div whileHover={{ scale: 1.05, rotateY: 180 }} className="inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-linear-to-br from-blue-600 via-purple-600 to-cyan-600 shadow-2xl shadow-blue-500/40 mb-6 relative">
            <Shield className="w-10 h-10 text-white" />
            <Sparkles className="w-5 h-5 text-yellow-300 absolute -top-2 -right-2" />
          </motion.div>
          <h1 className="text-5xl font-bold bg-linear-to-r from-gray-900 via-blue-900 to-purple-900 bg-clip-text text-transparent mb-4">Welcome Back</h1>
        ...
        <motion.div className="p-8 backdrop-blur-2xl bg-white/80 border border-white/50 shadow-2xl shadow-black/10 rounded-3xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Link href="/forgot-password">
                  <motion.span whileHover={{ scale: 1.05 }} className="text-sm bg-linear-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent font-semibold cursor-pointer">Forgot password?</motion.span>
                </Link>
              </div>
            ...
            <div className="text-center text-sm">
              ...
              <Link href="/sign-up">
                <motion.span whileHover={{ scale: 1.05 }} className="bg-linear-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent font-bold cursor-pointer">Create account</motion.span>
              </Link>
            </div>
          </form>
        </motion.div>
