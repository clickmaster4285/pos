...
import InvoiceSettingsForm from '@/components/setting/InvoiceSettingsForm';
...
export function CreateBillDialog({
  ...
  // Check if tax rate is defined and valid
  const isTaxRateSet = useMemo(() => {
    return companyData?.data?.invoiceSettings?.tax?.taxRate != null &&
           companyData.data.invoiceSettings.tax.taxRate > 0;
  }, [companyData]);

  // Update taxPercent from company settings when data is available
  useEffect(() => {
    if (isTaxRateSet) {
      setTaxPercent(companyData.data.invoiceSettings.tax.taxRate);
    }
  }, [companyData, isTaxRateSet, setTaxPercent]);
  ...
  // Memoize draftBill to prevent unnecessary recalculations
  const draftBill = useMemo(() => ({
    ...
    paymentMethod,
    status: 'paid',
  }), [buyer, items, subtotal, taxPercent, taxAmount, grandTotal, paymentMethod]);

  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="min-w-[70rem] max-h-[90vh] overflow-y-auto" aria-describedby="create-bill-description">
          <DialogHeader>
          ...
          <div className="space-y-6">
              ...
              <Card className="bg-card border-border lg:col-span-2">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="relative" ref={searchRef}>
                    ...
                    {showSearchResults && searchInventory.trim() && (
                      <div className="absolute top-full left-0 right-0 bg-background border border-border rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
                        {inventoryLoading ? (
                          ...
                          searchResults.map((item, idx) => (
                            <button
                              key={item.sku || idx} // Use SKU as key if available
              ...
              <Card className="bg-card border-border">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground" htmlFor="buyer-name">
                      Buyer Name
                    </label>
                    <Input
                      id="buyer-name"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground" htmlFor="buyer-email">
                      Email
                    </label>
                    <Input
                      id="buyer-email"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground" htmlFor="buyer-phone">
                      Phone
                    </label>
                    <Input
                      id="buyer-phone"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground" htmlFor="tax-percent">
                      Tax (%)
                    </label>
                    {isTaxRateSet ? (
                      <Input
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          aria-label="Open invoice settings to set tax rate"
                        >
            ...
            <div className="flex gap-3 justify-end pt-4 border-t">
              <Button variant="outline" onClick={() => onOpenChange(false)} aria-label="Cancel bill creation">
                Cancel
              </Button>
              <Button variant="outline" onClick={onReset} aria-label="Reset form">
                Reset Form
              </Button>
              <Button
                variant="outline"
                onClick={() => onPrintReceipt?.(draftBill)}
      ...
      <Dialog open={showInvoiceSettings} onOpenChange={setShowInvoiceSettings}>
        <DialogContent aria-describedby="invoice-settings-description">
          <DialogHeader>
            <DialogTitle>Invoice Settings</DialogTitle>
            <DialogDescription id="invoice-settings-description">
              Configure invoice settings including tax rate.
            </DialogDescription>
          </DialogHeader>
          <InvoiceSettingsForm companyId={companyId} />
        </DialogContent>
      </Dialog>
    </>
  );
