...
export default function InventoryDetailDialog({ open, onClose, item }) {
  ...
  const { data: inventoryData, isLoading } = useGetInventoryByIdQuery(item?.id || item?._id, {
    skip: !open || !item,
  });
  ...
  const downloadPDF = () => {
    ...
    const variantsData = inventoryItem.variants?.map(variant => [
      variant.variantName,
      variant.sku,
      variant.quantity,
    ...
    doc.save(`inventory-${inventoryItem.itemName}-${new Date().toISOString().split('T')[0]}.pdf`);
  };
    ...
    return (
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent>
          ...
          <div className="p-4 text-rose-700">Failed to load inventory details.</div>
        </DialogContent>
      </Dialog>
    );
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="min-w-[70rem] max-h-[95vh] overflow-y-auto" ref={contentRef}>
        <DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="border rounded-lg">
            <div className="p-4 border-b">
              <h3 className="text-lg font-semibold">Product Variants ({inventoryItem.totalVariants})</h3>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="text-left p-3 text-sm font-medium">Incoming</th>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="text-left p-3 text-sm font-medium">Performed By</th>
